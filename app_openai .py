...
from PIL import Image  # We'll use PIL to get image dimensions
from reportlab.pdfgen import canvas
...
def save_invoice():
    try:
        ...
        vat_option = request.form.get("vatOption", "").strip() if vat_amount else ""
        tax_amount = request.form.get("taxAmount", "").strip()
        tax_option = request.form.get("taxOption", "").strip() if tax_amount else ""
...
# Function to generate an invoice using a large language model with a given transcript
...
def generate_invoice(transcript):
    try:
        ...
        - End Note
        Do not require the user to specify phrases like "add", "include", etc. Automatically determine and insert any missing details.
        If any details are missing or unclear, use concise placeholders.
        ...
        response = openai.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are an AI that outputs only valid JSON without any extra text."},
                {"role": "user", "content": prompt}
            ],
            # response_format={"type": "json_object"},  # Forces JSON output from OpenAI API
            max_tokens=300,
            temperature=0.3,
        )
        if response and response.choices:
            invoice_content = response.choices[0].message.content.strip()
...
# Function to generate a professional PDF invoice using ReportLab
def generate_detailed_pdf(invoice_data):
    try:
        ...
        summary_items.append(("Subtotal:", f"{currency_symbol}{subtotal:.2f}"))
         ...
         # VAT
        vat_rate = safe_float(invoice_data['invoice'].get('vatAmount', 0))
        vat_option = invoice_data['invoice'].get('vatOption', '').lower()
        ...
        if vat_amount > 0:
         ...
         # Tax
        tax_rate = safe_float(invoice_data['invoice'].get('taxAmount', 0))
        tax_option = invoice_data['invoice'].get('taxOption', '').lower()
